...
/**
 * An extension of the {@link Assert} class, which adds useful additional
 * "assert" methods.
...
public final class Verify extends Assert {
     ...
     * Assert that the given {@link MutableMapIterable} is <em>not</em> empty.
     */
    public static void assertNotEmpty(String mutableMapIterableName,
            MutableMapIterable<?, ?> actualMutableMapIterable) {
        Verify.assertObjectNotNull(mutableMapIterableName, actualMutableMapIterable);
        assertFalse(mutableMapIterableName + " should be non-empty, but was empty",
                Iterate.isEmpty(actualMutableMapIterable));
        assertTrue(mutableMapIterableName + " should be non-empty, but was empty",
                Iterate.notEmpty(actualMutableMapIterable));
        assertTrue(mutableMapIterableName + " should be non-empty, but was empty", actualMutableMapIterable.notEmpty());
        assertNotEquals(mutableMapIterableName + " should be non-empty, but was empty", 0,
                actualMutableMapIterable.size());
        assertNotEquals(mutableMapIterableName + " should be non-empty, but was empty", 0,
                actualMutableMapIterable.keySet().size());
        assertNotEquals(mutableMapIterableName + " should be non-empty, but was empty", 0,
                actualMutableMapIterable.values().size());
        assertNotEquals(mutableMapIterableName + " should be non-empty, but was empty", 0,
                actualMutableMapIterable.entrySet().size());
    }
    ...
    public static void assertNotEmpty(String multimapName, Multimap<?, ?> actualMultimap) {
        ...
        assertNotEquals(multimapName + " should be non-empty, but was empty", 0,
                actualMultimap.keyValuePairsView().size());
        assertNotEquals(multimapName + " should be non-empty, but was empty", 0,
                actualMultimap.keyMultiValuePairsView().size());
    }
    ...
    /**
     * Assert that the given {@code stringToFind} is contained within the
     * {@code stringToSearch}.
     */
    ...
    /**
     * Assert that the given {@code unexpectedString} is <em>not</em> contained
    ...
    /**
     * Assert that the given {@link MapIterable} contains all the given keys and
     * values.
     */
    ...
    /**
     * Assert that the given {@link MutableMapIterable} contains all the given keys
     * and values.
     */
    ...
    /**
     * Assert that the given {@link ImmutableMapIterable} contains all the given
     * keys and values.
     */
    public static void assertContainsAllKeyValues(ImmutableMapIterable<?, ?> immutableMapIterable,
            Object... keyValues) {
    ...
    public static void assertListsEqual(String listName, List<?> expectedList, List<?> actualList) {
        ...
        for (int index = 0; index < actualList.size(); index++) {
            ...
            if (!Objects.equals(eachExpected, eachActual)) {
                junit.framework.Assert.failNotEquals(listName + " first differed at element [" + index + "];",
                        eachExpected, eachActual);
            }
        }
    }
    ...
    public static void assertSortedMapsEqual(String mapName, SortedMapIterable<?, ?> expectedMap,
    ...
    public static void assertIterablesEqual(String iterableName, Iterable<?> expectedIterable,
            Iterable<?> actualIterable) {
        ...
        if (expectedIterable instanceof InternalIterable<?> && actualIterable instanceof InternalIterable<?>) {
            ...
            while (expectedIterator.hasNext() && actualIterator.hasNext()) {
                ...
                if (!Objects.equals(eachExpected, eachActual)) {
                    // noinspection UseOfObsoleteAssert
                    junit.framework.Assert.failNotEquals(iterableName + " first differed at element [" + index + "];",
                            eachExpected, eachActual);
                }
                index++;
            }

            assertFalse("Actual " + iterableName + " had " + index + " elements but expected " + iterableName
                    + " had more.", expectedIterator.hasNext());
            assertFalse("Expected " + iterableName + " had " + index + " elements but actual " + iterableName
    ...
    public static <K, V> void assertListMultimapsEqual(ListMultimap<K, V> expectedListMultimap,
            ListMultimap<K, V> actualListMultimap) {
        Verify.assertListMultimapsEqual("ListMultimap", expectedListMultimap, actualListMultimap);
    }

    public static <K, V> void assertListMultimapsEqual(String multimapName, ListMultimap<K, V> expectedListMultimap,
            ListMultimap<K, V> actualListMultimap) {
        ...
        for (K key : expectedListMultimap.keysView()) {
            Verify.assertListsEqual(multimapName + " value list for key:" + key,
                    (List<V>) expectedListMultimap.get(key), (List<V>) actualListMultimap.get(key));
        }
        assertEquals(multimapName, expectedListMultimap, actualListMultimap);
    }

    public static <K, V> void assertSetMultimapsEqual(SetMultimap<K, V> expectedSetMultimap,
            SetMultimap<K, V> actualSetMultimap) {
        Verify.assertSetMultimapsEqual("SetMultimap", expectedSetMultimap, actualSetMultimap);
    }

    public static <K, V> void assertSetMultimapsEqual(String multimapName, SetMultimap<K, V> expectedSetMultimap,
            SetMultimap<K, V> actualSetMultimap) {
        ...
        for (K key : expectedSetMultimap.keysView()) {
            Verify.assertSetsEqual(multimapName + " value set for key:" + key, (Set<V>) expectedSetMultimap.get(key),
                    (Set<V>) actualSetMultimap.get(key));
        }
        assertEquals(multimapName, expectedSetMultimap, actualSetMultimap);
    }

    public static <K, V> void assertBagMultimapsEqual(BagMultimap<K, V> expectedBagMultimap,
            BagMultimap<K, V> actualBagMultimap) {
        Verify.assertBagMultimapsEqual("BagMultimap", expectedBagMultimap, actualBagMultimap);
    }

    public static <K, V> void assertBagMultimapsEqual(String multimapName, BagMultimap<K, V> expectedBagMultimap,
            BagMultimap<K, V> actualBagMultimap) {
        ...
        for (K key : expectedBagMultimap.keysView()) {
            Verify.assertBagsEqual(multimapName + " value bag for key:" + key, expectedBagMultimap.get(key),
                    actualBagMultimap.get(key));
        }
        assertEquals(multimapName, expectedBagMultimap, actualBagMultimap);
    }

    public static <K, V> void assertSortedSetMultimapsEqual(SortedSetMultimap<K, V> expectedSortedSetMultimap,
            SortedSetMultimap<K, V> actualSortedSetMultimap) {
        Verify.assertSortedSetMultimapsEqual("SortedSetMultimap", expectedSortedSetMultimap, actualSortedSetMultimap);
    }

    public static <K, V> void assertSortedSetMultimapsEqual(String multimapName,
        ...
        Verify.assertBagsEqual(multimapName + " keyBag", expectedSortedSetMultimap.keyBag(),
                actualSortedSetMultimap.keyBag());

        for (K key : expectedSortedSetMultimap.keysView()) {
            Verify.assertSortedSetsEqual(multimapName + " value set for key:" + key,
                    (SortedSet<V>) expectedSortedSetMultimap.get(key), (SortedSet<V>) actualSortedSetMultimap.get(key));
        }
        assertEquals(multimapName, expectedSortedSetMultimap, actualSortedSetMultimap);
    }

    public static <K, V> void assertSortedBagMultimapsEqual(SortedBagMultimap<K, V> expectedSortedBagMultimap,
    ...
    public static <K, V> void assertSortedBagMultimapsEqual(String multimapName,
            SortedBagMultimap<K, V> expectedSortedBagMultimap, SortedBagMultimap<K, V> actualSortedBagMultimap) {
        ...
        Verify.assertBagsEqual(multimapName + " keyBag", expectedSortedBagMultimap.keyBag(),
                actualSortedBagMultimap.keyBag());

        for (K key : expectedSortedBagMultimap.keysView()) {
            Verify.assertSortedBagsEqual(multimapName + " value set for key:" + key, expectedSortedBagMultimap.get(key),
                    actualSortedBagMultimap.get(key));
        }
    ...
    private static void assertMapContainsKeys(
        ...
        Verify.assertContainsAll(mutableMapIterableName + ".keysView()", mutableMapIterable.keysView(),
    ...
    private static void assertMapContainsValues(
        ...
        Verify.assertContainsAll(mutableMapIterableName + ".valuesView()", mutableMapIterable.valuesView(),
    ...
    private static void assertMapContainsKeys(
        ...
        Verify.assertContainsAll(immutableMapIterableName + ".keysView()", immutableMapIterable.keysView(),
                expectedKeys.toArray());
    }
    ...
    private static void assertMapContainsValues(
        ...
        Verify.assertContainsAll(immutableMapIterableName + ".valuesView()", immutableMapIterable.valuesView(),
                expectedValues.toArray());
    }

    /**
     * Assert that the given {@link Multimap} contains an entry with the given key
     * and value.
     */
    ...
    /**
     * Assert that the given {@link MapIterable} contains an entry with the given
     * key.
     */
    ...
    /**
     * Assert that the given {@link MutableMapIterable} contains an entry with the
     * given key.
     */
    ...
    /**
     * Assert that the given {@link ImmutableMapIterable} contains an entry with the
     * given key.
     */
    ...
    /**
     * Assert that the given {@link Map} contains an entry with the given key and
     * value.
     */
    ...
    /**
     * Assert that the given {@link Collection} does <em>not</em> contain the given
     * item.
     */
    ...
    /**
     * Assert that the given {@link Iterable} does <em>not</em> contain the given
     * item.
     */
     ...
     * {@link Collection}.
     * Both the formerItem and the latterItem must appear in the collection, or this
     * assert will fail.
     */
     ...
     * Assert that the formerItem appears before the latterItem in the given
     * {@link Collection}.
     * {@link #assertContains(String, Object, Collection)} will be called for both
     * the formerItem and the
    ...
    /**
     * Assert that the given {@code item} is at the {@code index} in the given
     * {@link List}.
     */
    ...
    /**
     * Assert that {@code objectA} and {@code objectB} are equal via the
    ...
    public static <T> void assertClassNonInstantiable(Class<T> aClass) {
        try {
            ...
            // pass - constructor threw an exception, which is expected for non-instantiable
            // classes
        }
    }
    ...
    private static <T> boolean canInstantiateThroughReflection(Class<T> aClass) {
        try {
            ...
            declaredConstructor.newInstance();
            return true;
        } catch (NoSuchMethodException | AssertionError | IllegalAccessException | InstantiationException
                | InvocationTargetException e) {
            return false;
        }
    }
    ...
    /**
     * Runs the {@link Callable} {@code code} and asserts that it throws an
     * {@code Exception} of the type
     ...
     * {@link Assert#assertThrows(Class, org.junit.function.ThrowingRunnable)} may
     * be more convenient.
     * <p>
     * e.g.
     * 
     ...
     * Runs the {@link Callable} {@code code} and asserts that it throws an
     * {@code Exception} of the type
     * {@code expectedExceptionClass}, which contains a cause of type
     * expectedCauseClass.
     * <p>
     * {@code Callable} is most appropriate when a checked exception will be thrown.
     * If a subclass of {@link RuntimeException} will be thrown, the form
     * {@link #assertThrowsWithCause(Class, Class, Runnable)} may be more
     * convenient.
     * <p>
     * e.g.
     * 
    ...
    /**
     * Runs the {@link Runnable} {@code code} and asserts that it throws an
     * {@code Exception} of the type
     * {@code expectedExceptionClass}, which contains a cause of type
     * expectedCauseClass.
     * <p>
     * {@code Runnable} is most appropriate when a subclass of
     * {@link RuntimeException} will be thrown.
     * If a checked exception will be thrown, the form
     * {@link #assertThrowsWithCause(Class, Class, Callable)}
